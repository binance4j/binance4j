...
public class SymbolFiltersDeserializer extends JsonDeserializer<SymbolFilters> {
	...
	public SymbolFilters deserialize(JsonParser jp, DeserializationContext ctx) throws IOException {
		...
		node.forEach(n -> {
			...
			case "LOT_SIZE" -> lotSizeFilter = new LotSizeFilter(n.get("minQty").asText(), n.get("maxQty").asText(), n.get("stepSize").asText());
			case "MARKET_LOT_SIZE" -> marketLotSizeFilter = new MarketLotSizeFilter(n.get("minQty").asText(), n.get("maxQty").asText(),
					n.get("stepSize").asText());
			case "MAX_NUM_ALGO_ORDERS" -> maxNumAlgoOrdersFilter = new MaxNumAlgoOrdersFilter(n.get("maxNumAlgoOrders").asText());
			...
			case "MIN_NOTIONAL" -> minNotionalFilter = new MinNotionalFilter(n.get("minNotional").asText(), n.get("applyToMarket").asBoolean(),
					n.get("avgPriceMins").asInt());
			case "NOTIONAL" -> notionalFilter = new NotionalFilter(n.get("minNotional").asText(), n.get("maxNotional").asText(),
					n.get("applyMinToMarket").asBoolean(), n.get("applyMaxToMarket").asBoolean(), n.get("avgPriceMins").asInt());
			case "PERCENT_PRICE_BY_SIDE" -> percentPriceBySideFilter = new PercentPriceBySideFilter(n.get("bidMultiplierUp").asText(),
					n.get("bidMultiplierDown").asText(), n.get("askMultiplierUp").asText(), n.get("askMultiplierDown").asText(),
					n.get("avgPriceMins").asText());
			case "PERCENT_PRICE" -> percentPriceFilter = new PercentPriceFilter(n.get("multiplierUp").asText(), n.get("multiplierDown").asText(),
					n.get("avgPriceMins").asText());
			case "PRICE_FILTER" -> priceFilter = new PriceFilter(n.get("minPrice").asText(), n.get("maxPrice").asText(), n.get("tickSize").asText());
			case "TRAILING_DELTA" -> trailingDeltaFilter = new TrailingDeltaFilter(n.get("minTrailingAboveDelta").asText(),
					n.get("maxTrailingAboveDelta").asText(), n.get("minTrailingBelowDelta").asText(), n.get("maxTrailingBelowDelta").asText());
			default -> {
			}
			}
		});
