...
Every `WebsocketClient` can be configured with a `WebsocketClientConfiguration`.
...
<table>
    ...
    <tr class="fw-400">
        ...
        <th>Duration</th>
        <th>
            Time after which the client will disconnect. If <code>keepAlive</code> is true, will reconnect. Prevents
            that the server closes itself the stream.
        </th>
        <th>1d</th>
    </tr>
</table>
