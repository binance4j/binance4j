...
As you can see in the example above, all exceptions are handled with `ApiException`. Synchronous calls will throw an `ApiException` and asynchronous calls will pass the exception to the `onFailure` method of the `ApiCallback`. Using a lambda in an async callback doesn't permir to handle such exception.
