...
public class FiatClientTest extends CustomTest {
	...
	@Override
	protected FiatClient getClient() {
		return client;
	}

	@Test
	void testGetPayments() throws ApiException, InterruptedException {
		testNotThrow(getClient().getPayments(new PaymentParams(PaymentType.SELL)));
	}
	...
	void testGetPayments2() throws ApiException, InterruptedException {
		...
		testNotThrow(getClient().getPayments(new PaymentParams(PaymentType.BUY), new FramedPaging(1)));
	...
	void testGetTransactions() throws ApiException, InterruptedException {
		...
		testNotThrow(getClient().getTransactions(new TransactionParams(TransactionType.WITHDRAW)));
	}

	@Test
	void testGetTransactions2() throws ApiException, InterruptedException {
		TimeUnit.SECONDS.sleep(30);
		testNotThrow(getClient().getTransactions(new TransactionParams(TransactionType.DEPOSIT), new FramedPaging(1)));
	}
}
