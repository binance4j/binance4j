...
public class CandleDeserializer extends JsonDeserializer<Candle> {
	...
	public Candle deserialize(JsonParser jp, DeserializationContext ctx) throws IOException {
		...
		return new Candle(node.get(0).asLong(), node.get(1).asText(), node.get(2).asText(), node.get(3).asText(),
				node.get(4).asText(), node.get(5).asText(),
				node.get(6).asLong(), node.get(7).asText(), node.get(8).asLong(), node.get(9).asText(),
				node.get(10).asText());
	}
}
